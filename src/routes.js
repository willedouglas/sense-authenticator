...
module.exports = app => { 
  ...
  app.post('/session/login', (req, res) => {
    QRS.createSession(req.body.username, (error, session) => {
      if (session) {
        //res.status(200).json({ status: true, result: { message: 'Sessão ' + session.SessionId + ' criada com sucesso.', session: session.SessionId }});
        return res.cookie('X-Qlik-Session-Hefesto', session.SessionId);
      }
      return res.status(500).json({ status: false, error: error });
    });
  });
